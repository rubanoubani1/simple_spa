...
addToList = async () =>{
	...
	if(response.ok){
		...
		console.log("Server reponed with a status:",response.status);
	}
...
getContactList = async()=>{
	...
	if(response.ok){
		...
		console.log("Failed to load contacts. Server respomded with a status:", response.status)
	}
...
populateTable = (data) => {
	...
	removeheader.appendChild(edittext);
	...
	for(let i=0;i<data.length;i++){
		...
		editButton.addEventListener("click", function(event){
			console.log("this will edit object", data[i].toString())
		})
